...
def place_icons01(image, size):
    icon_name = ["rect" , "circle", "eclipe", "line",   # 아이콘 파일 이름
                 "brush", "eraser", "open"  , "save",
                 "plus" , "minus" , "histo", 'mosaic' ,                  # 연습문제 수정
                 "clear" , "color"]
...
def place_icons03(image, size):
    icon_name = ["rect" , "circle", "eclipe", "line",   # 아이콘 파일 이름
                 "brush", "eraser", "open"  , "save",
                 "plus" , "minus" , "up", 'down' ,                  # 연습문제 수정
                 "clear" , "color"]
...
# 세로로 긴 영역  15x120
def create_hueIndex06(image, roi):
    x, y, w, h =  roi                         # 관심영역 너비, 높이
    index = [[(j, 1, 1) for i in range(w)] for j in range(h)]      # 가로로 만들기
    ratios = (180 / h, 255, 255)
    hueIndex = np.multiply(index, ratios).astype('uint8')  # HSV 화소값 행렬

    image[y:y+h, x:x+w] = cv2.cvtColor(hueIndex, cv2.COLOR_HSV2BGR)

def create_colorPlatte06(image, idx, roi):
    x, y, w, h = roi
    hue = idx-y

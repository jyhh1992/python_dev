...
def onMouse(event, x, y, flags, param):
    ...
    if mouse_mode >= 2:  # 왼쪽 버튼 누르기 또는 드래그
        mouse_mode = 0 if y < 125 else 3  # 메뉴 영역 확인- 마우스 상태 지정
        pt2 = (x, y)
...
def command(mode):
    ...
    if mode == PALETTE:  # 색상팔레트 영역 클릭 시
        ...
        create_colorPlatte06(image, pt2[1], icons[PALETTE])  # 팔레트 새로 그리기
...
image = np.full((600, 900, 3), 255, np.uint8)         # 연습문제 수정
icons = place_icons06(image, (60, 60))
x, y, w, h = icons[-1]

icons.append((x+ w + 2  , 0, 120, 120) )      # 팔레트 사각형 추가
icons.append((x+ w + 124, 0, 15, 120))  # 색상인덱스 사각형 추가
create_colorPlatte06(image, 0, icons[PALETTE])    # 팔레트 생성
create_hueIndex06(image, icons[HUE_IDX])                      # 색상인텍스 생성

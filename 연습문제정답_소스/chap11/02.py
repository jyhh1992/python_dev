...
def command(mode):
    ...
    if mode == PALETTE:  # 색상팔레트 영역 클릭 시
        ...
        bgr = cv2.split(canvas)
        ch, hsize, ranges = [0], [32], [0, 256]
        title  = ['blue', 'green','red']
        for c, t in zip(bgr, title):
            hist = cv2.calcHist([c], ch, None, hsize, ranges)
            cv2.imshow('histo_'+ t , draw_histo(hist))

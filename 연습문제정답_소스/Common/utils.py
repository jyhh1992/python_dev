...
def ck_time(mode = 0 , msg = ""):
    ...
    if (mode ==0 ):
       ...
       elapsed = (etime - stime)
       print("elapsed time = %.5f sec" % elapsed)  # 초 단위 경과 시간
...
# def contain_roi(p, roi):
#     return roi[0] <= p[0] < roi[0]+ roi[2] and roi[1]<= p[1] < roi[1] + roi[3]
#
# def get_size(image):
#     return (image.shape[1], image.shape[0])

# 영역 추출 - 사각형(시작점, 크기)으로 관심영역 추출
# def get_roi(image, roi):
#     x, y, w, h = roi
#     return image[y:y + h, x:x + w]

# def get_roi_pt(image, p1, p2):
#     (x, y), (w, h) = p1, np.subtract(p2,p1)
#     return image[y:y + h, x:x + w]
